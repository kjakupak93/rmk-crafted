...
| `activity_log` | Recent activity feed — columns: `id`, `icon` (emoji text), `name` (entity name), `sub` (detail line), `created_at`. Requires `authenticated` role. Populated by `logActivity()`. |
...
- `logActivity(icon, name, sub)` — fire-and-forget async helper that inserts a row into `activity_log`. Wrapped in try/catch internally — a DB failure logs a console warning but never propagates to callers. Safe to bare-await anywhere in a save function.
- `_withSaving(btnId, fn)` — async helper that disables a modal save button for the duration of `fn`, re-enables it in `finally`. Applied to all 7 modal save buttons to prevent double-submission. Guards re-entrancy via `btn.disabled` check.
...
- Payment methods: `cash`, `venmo`, `other`, `unpaid`. Rendered as `badge-cash`, `badge-venmo`, `badge-other`, `badge-unpaid`. Log Sale modal `#sPayment` dropdown includes "Other" option. `badge-other` uses `rgba(255,255,255,0.06)` background with `--text-muted` color.
- Quotes tab action buttons: icon-only buttons (`icon-btn` class) — Convert (⚡, `title="Convert to Order"`), Edit (✏️, `title="Edit"`), Delete (🗑️, `title="Delete"`, red color via `var(--red)`).
